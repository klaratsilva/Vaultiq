...
function stripLocaleFromPath(path: string) {
  const parts = path.split("/").slice(2); // remove "", "en", "..."
  return "/" + parts.join("/");
}
...
export default function Navbar() {
  ...
  return (
    <section className="sidebar flex h-full flex-col bg-white border-r w-64">
      <nav className="flex flex-1 flex-col gap-4 px-4 py-6">
        {/* Logo → Home */}
        <Link
          href="/"
          locale={locale} // Next.js will turn this into "/en" or "/fr"
        ...
        >
          ...
          />
          <h1 className="text-lg font-bold">VaultIq</h1>
        </Link>

        {sidebarLinks.map((item: SidebarItem) => {
          // item.route is the “base” ("/" or "/accounts", etc.)
          // Determine active state by comparing basePath to item.route
          ...
          return (
            <Link
              key={item.labelKey}
              href={item.route}
              locale={locale} // Next.js will generate "/{locale}{item.route}"
